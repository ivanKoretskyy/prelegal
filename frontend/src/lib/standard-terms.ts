...
/**
 * Verbatim from templates/Mutual-NDA.md (Common Paper Mutual NDA, Standard
 * Terms v1.0, CC BY 4.0). `<span class="coverpage_link">Label</span>` marks
 * a value that's resolved from the Cover Page.
 */
const RAW_PARAGRAPHS: string[] = [
...
export type InlineToken =
  ...
  | { type: "blank"; field: FieldKey; label: string };

const TOKEN_PATTERN = /\*\*(.+?)\*\*|<span class="coverpage_link">(.+?)<\/span>/g;
...
function parseInline(text: string): InlineToken[] {
  ...
  while ((match = TOKEN_PATTERN.exec(text)) !== null) {
    ...
    const [, boldValue, blankLabel] = match;
    if (boldValue !== undefined) {
      tokens.push({ type: "bold", value: boldValue });
    } else if (blankLabel !== undefined) {
      tokens.push({ type: "blank", field: LABEL_TO_FIELD[blankLabel], label: blankLabel });
    }
...
export type StandardTermsParagraph = {
  id: string;
  tokens: InlineToken[];
};

export const STANDARD_TERMS_PARAGRAPHS: StandardTermsParagraph[] = RAW_PARAGRAPHS.map(
  (paragraph, index) => ({
    id: `term-${index + 1}`,
